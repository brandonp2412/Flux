...
Workspace roots may additionally declare `[workspace].members = ["apps/demo", "libs/core"]`. Entries are explicit, relative directories inside the root; each must contain a manifest, member package names must be unique (including against the root package), and nested workspace declarations are rejected. Workspace dependency resolution writes one root `flux.lock`: member dependency IDs are qualified by the member package name, and member-targeted commands validate/replay that root lock. `flux check`, `flux analyze`, `flux build`, and `flux test` on the root operate deterministically across the root and every member. Members are not implicitly importable; source imports still require an explicitly declared dependency, so a same-named registry package cannot be silently shadowed by a workspace directory.
...
`flux fetch <package-dir|flux.toml> [--offline]` resolves the transitive registry graph to one highest compatible non-yanked version per package and fills the global content-addressed cache through the public registry by default or an explicit provider override; exact immutable releases already committed in `flux.lock` remain replayable without provider metadata. `flux vendor <package-dir|flux.toml> [-o directory] [--offline]` is the explicit self-contained snapshot escape hatch: it materializes the already-verified graph under `packages/<name>/<version>/` and writes deterministic `flux.vendor.lock` metadata. Vendoring is never the default dependency layout. Registry graph resolution includes registry dependencies reached through local path dependencies, enforces each release's Flux compatibility requirement, and rejects registry dependency cycles. Revision-pinned Git dependencies resolve to exact commits, are archived through the same canonical `.fluxpkg` producer, enter the same SHA-256-addressed cache, and can be fetched or vendored again in strict offline mode from their immutable lock identity.

Static registry release metadata is compiler-owned format version 1. Each package/version record contains package name, owner, HTTPS source repository, exact SemVer version, Flux compatibility requirement, immutable HTTPS release-asset URL, SHA-256, dependency requirements, and a yank flag. The parser is strict: unknown fields such as install/build hooks fail rather than being ignored. Registry lookup defaults to the public static index at `https://raw.githubusercontent.com/brandonp2412/flux-registry/main`; `FLUX_REGISTRY_DIR` selects a checked-out/private/test index and `FLUX_REGISTRY_URL` selects an alternate static HTTPS mirror whose `<package>/versions.txt` lists versions and whose `<package>/<version>.toml` stores release metadata. The checked-in [`registry/`](../registry/) tree documents and validates the public-index layout, while the canonical hosted index lives at `https://github.com/brandonp2412/flux-registry`; neither contains package archives or executable publication hooks. Static metadata is cached under the Flux package cache so `--offline` never contacts the configured URL. Provider resolution feeds exact release metadata into lock generation and ordinary package imports. Existing exact registry lock entries are provider-independent: ordinary imports and fetch can replay them without the original provider.

`flux publish package <package-dir|flux.toml> [--repo owner/name] [--registry owner/name]` is the bootstrap GitHub publication path. It requires authenticated `gh`, refreshes and validates the package lock, checks the package, runs its Flux tests, requires an explicit package version, permits only registry SemVer dependencies, and rejects native plugin/library/search-path declarations rather than allowing an unaudited native package boundary. The publisher creates the canonical `{name}-{version}.fluxpkg`, uploads it to the package repository's `v{version}` GitHub Release, and treats a pre-existing release as a valid retry only when downloading the existing asset produces the identical SHA-256. It then clones `brandonp2412/flux-registry` by default (`--registry` or `FLUX_REGISTRY_GITHUB_REPO` may override it), forks it when necessary, writes deterministic `<package>/<version>.toml` plus `versions.txt`, and submits a pull request. A published version's metadata is immutable: an identical re-write is idempotent, while different metadata for the same version fails. `<package>/owner.txt` is the explicit package-name ownership gate; changing ownership requires a deliberate registry change before a different GitHub owner can publish a new version. Yank state remains registry metadata, so new resolution can exclude a broken version without deleting the immutable release bytes needed by existing locks.

## Reproducible lockfile

Packages with dependencies use a compiler-owned `flux.lock`. Run `flux lock <package-dir|flux.toml>` after changing dependency declarations or resolved local package metadata. Package-aware analysis/build commands refresh a missing or stale deterministic lock by default; `--locked` rejects it without modification for CI/reproducible builds.

Lockfile format version 1 is generated deterministically. Entries are ordered by dependency path through the package graph. Loadable path dependencies record the declared relative source plus the resolved dependency package name and exact package version when one is declared; transitive path dependencies are included recursively. Git declarations record their immutable revision. When registry dependencies resolve through the default public index or an explicit provider override, registry entries record the exact selected version, immutable repository/release identity, release-asset URL, and SHA-256. Those exact entries are parsed and structurally validated on later builds without re-resolving: manifest requirements must still accept the locked version, immutable identity/hash fields must remain valid, and stale exact locks are never silently downgraded when provider metadata is unavailable. Exact locks can replay without contacting the public index because they retain the immutable release identity and content hash. Re-running `flux lock` with unchanged inputs produces byte-identical output. Editing a transitive path dependency's package metadata makes the root lockfile stale and requires regeneration before the package can load again.
...
`flux new` emits the current package-format version. `flux lock` writes the dependency lockfile described above. `flux fetch` and `flux vendor` use the registry/provider/cache contract above, including strict `--offline` operation, while `flux publish package` performs the authenticated GitHub Release plus registry-PR workflow described above. Package-aware commands such as `flux check`, `flux build`, `flux run`, `flux test`, and `flux package` all use the same manifest parser and lock validation. Ordinary online analysis resolves through the public static registry by default or an explicit provider override, checksum-verifies the selected graph, materializes exact package sources under the global content-addressed cache, and loads `pkg:` imports directly from those verified roots. Exact entries already committed to `flux.lock` remain provider-independent; `flux fetch --offline` uses only those locked identities plus verified cache contents and never contacts the public index.

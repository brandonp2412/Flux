...
Workspace roots may additionally declare `[workspace].members = ["apps/demo", "libs/core"]`. Entries are explicit, relative directories inside the root; each must contain a manifest, member package names must be unique (including against the root package), and nested workspace declarations are rejected. Members are metadata identities only at this stage: they are not implicitly importable and still require the workspace-wide lock/check/build orchestration tracked in the roadmap.

...
Already-obtained archives can be placed in the Flux package cache by their lowercase SHA-256 digest. Cache insertion verifies the source bytes before storing them, and cache lookup re-hashes the stored archive before returning it; corrupt or tampered entries fail closed. `FLUX_PACKAGE_CACHE_DIR` selects the package-cache root explicitly, otherwise package storage lives below the existing Flux/XDG cache location. Verified extraction rejects absolute/parent-traversal archive entries plus symbolic/hard links, extracts into a temporary sibling directory, requires a root `flux.toml`, and only then atomically installs the source tree. Registry release materialization is cache-first: online fetching uses the immutable metadata asset URL and verifies SHA-256 before admitting bytes to the cache, while strict offline mode reads only cached registry metadata plus verified cached archives and never attempts a network request.

`flux fetch <package-dir|flux.toml> [--offline]` resolves the transitive registry graph to one highest compatible non-yanked version per package and fills the global content-addressed cache. `flux vendor <package-dir|flux.toml> [-o directory] [--offline]` is the explicit self-contained snapshot escape hatch: it materializes the already-verified graph under `packages/<name>/<version>/` and writes deterministic `flux.vendor.lock` metadata. Vendoring is never the default dependency layout. Registry graph resolution includes registry dependencies reached through local path dependencies, enforces each release's Flux compatibility requirement, and rejects registry dependency cycles. Revision-pinned Git fetch transport remains separate follow-up work.

Static registry release metadata is compiler-owned format version 1. Each package/version record contains package name, owner, HTTPS source repository, exact SemVer version, Flux compatibility requirement, immutable HTTPS release-asset URL, SHA-256, dependency requirements, and a yank flag. The parser is strict: unknown fields such as install/build hooks fail rather than being ignored. Registry lookup is behind a provider interface: `FLUX_REGISTRY_DIR` selects a deterministic checked-out/local mirror, while `FLUX_REGISTRY_URL` selects a static HTTPS index whose `<package>/versions.txt` lists versions and whose `<package>/<version>.toml` stores release metadata. Static metadata is cached under the Flux package cache so `--offline` never contacts the configured URL. Configured providers feed exact release metadata into lock generation and ordinary package imports; a public GitHub-backed index plus provider-independent replay directly from `flux.lock` remain follow-up work.
...
`flux new` emits the current package-format version. `flux lock` writes the dependency lockfile described above. `flux fetch` and `flux vendor` use the registry/provider/cache contract above, including strict `--offline` operation. Package-aware commands such as `flux check`, `flux build`, `flux run`, `flux test`, and `flux package` all use the same manifest parser and lock validation. When a registry provider is configured, their ordinary analysis path resolves and checksum-verifies the selected graph, materializes exact package sources under the global content-addressed cache, and loads `pkg:` imports directly from those verified roots. Provider-independent/offline replay from only the exact lockfile remains follow-up work.

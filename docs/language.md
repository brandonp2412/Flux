...
Existing list/view operations model the bootstrap's first immutable-borrow behavior: reading, indexing, slicing, iteration, matching, sequence transforms, and passing `T[]` to a function do not consume the local list owner. Zero-copy views retain normalized borrow provenance through ordinary local bindings and through expression-scoped list-match bindings, so a nested view selected by a value-producing `match` keeps its outer owner immovable until that view's final reachable use. Local nested-list literals also preserve this provenance when they store a non-copy list descriptor, preventing an embedded list owner from moving while the nested aggregate remains live. These are compiler-enforced local views, not yet first-class source-level borrow/reference types, and they cannot escape through list returns or struct/enum aggregate storage.

...
`path.isAbsolute(value)` provides a zero-allocation native path-shape query. `path.join(base, child, callback)` joins two borrowed path strings and lends the result only for the exact callback invocation. `path.dirname(value, callback)` and `path.basename(value, callback)` similarly lend the lexical parent/name component. An absolute child replaces the base; otherwise one native separator is inserted when needed. Component and join operations use fixed 65537-byte stack buffers, reject empty/NUL/overlong inputs and results, and emit no runtime helper when unreachable. This bootstrap slice accepts both slash forms for cross-platform path spelling; platform-specific normalization and owned/escaping path values remain future ownership work.

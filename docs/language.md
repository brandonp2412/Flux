...
`path.isAbsolute(value)` provides a zero-allocation native path-shape query. `path.join(base, child, callback)` joins two borrowed path strings and lends the result only for the exact callback invocation. An absolute child replaces the base; otherwise one native separator is inserted when needed. The operation uses a fixed 65537-byte stack buffer, rejects NULs and overlong inputs/results, and emits no runtime helper when unreachable. This bootstrap slice accepts both slash forms for cross-platform path spelling; platform-specific normalization and owned/escaping path values remain future ownership work.

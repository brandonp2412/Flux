...
Linux/server applications can wrap a connected native socket with `tls.wrap(socket, serverName, caFile)`, which performs OpenSSL certificate-chain and hostname verification using the supplied CA bundle (or system defaults when `caFile` is empty). `tls.read(session, maxBytes, callback)`, `tls.write(session, value)`, and `tls.close(session)` use bounded borrowed text and explicit `error` results; sessions own and release their wrapped socket on close. The TLS helper is tree-shaken when unreachable. Nonblocking/async TLS scheduling, server certificates, session resumption, and mobile providers remain future work.

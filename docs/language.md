...
Portable one-action alerts use `dialog.alert(title: str, message: str) -> void`. The capability is application-only, exposes no dialog/controller object, and tree-shakes when unreachable. Linux lowers it to a native modal GTK message dialog owned by the active application window; Android lowers it directly through compiler-owned JNI to the platform `AlertDialog.Builder` with the standard OK dismissal action. Rich actions, sheets, custom dialog content, and dialog-as-navigation semantics remain separate UI/navigation work rather than being hidden behind this narrow alert API.

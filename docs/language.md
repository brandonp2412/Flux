...
The Linux/server `crypto.sha256(value, callback)` capability computes a lowercase SHA-256 digest through the platform OpenSSL/libcrypto implementation and lends the 64-character hexadecimal result only during the synchronous callback. Inputs are bounded to 65536 bytes, failures return `error`, and the helper is tree-shaken when unreachable. This is the first audited-library cryptographic primitive; additional primitives and mobile/portable provider contracts remain future work.

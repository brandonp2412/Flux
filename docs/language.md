...
Windows distribution can produce an unsigned MSIX bundle with `flux package --format msix --target <windows-triple>`. The bundle contains the native executable, staged assets, a versioned full-trust `AppxManifest.xml`, and compiler-generated placeholder tile images. Signing certificates, package identity provisioning, and Microsoft Store submission are intentionally separate release-workflow steps.

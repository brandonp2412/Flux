...
Windows bitmap `Image` sources use compiler-owned Win32 `LoadImageA`/`STM_SETIMAGE` handling with refresh-time bitmap lifetime cleanup; packaged `asset://` sources resolve beneath the executable-adjacent `assets/` directory with traversal rejection. Compile-time Flux hex and semantic colors for Text/backgrounds lower through native `WM_CTLCOLOR*` brushes, with compiler-owned cleanup; generated GUI C is syntax-validated and linked into a Windows executable through available Wine Win32 headers/import libraries, while interactive native Windows execution and dynamic styling remain pending.

...
Portable one-action alerts use `dialog.alert(title: str, message: str) -> void`. Typed confirmation uses `dialog.confirm(title: str, message: str, onConfirm: fn() -> void, *, cancelLabel: str = "Cancel", confirmLabel: str = "OK") -> void`; the optional named labels customize the two native actions without introducing a dialog/controller object, and only acceptance invokes the callback. Both capabilities are application-only and tree-shake when unreachable. Linux lowers them to native modal GTK dialogs owned by the active application window; Android lowers them directly through compiler-owned JNI to the platform `AlertDialog.Builder`. Multi-action choices, sheets, custom dialog content, and dialog-as-navigation semantics remain separate UI/navigation work rather than being hidden behind these narrow APIs.

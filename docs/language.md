...
Windows bitmap `Image` sources use compiler-owned Win32 `LoadImageA`/`STM_SETIMAGE` handling with refresh-time bitmap lifetime cleanup; `asset://` sources remain deferred until Windows asset packaging exists.

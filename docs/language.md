...
The Linux/server `crypto.sha256(value, callback)` and `crypto.hmacSha256(key, value, callback)` capabilities compute lowercase hexadecimal SHA-256 digests through the platform OpenSSL/libcrypto implementation and lend the 64-character result only during the synchronous callback. Each input is bounded to 65536 bytes, failures return `error`, and helpers are tree-shaken when unreachable. Additional primitives and mobile/portable provider contracts remain future work.

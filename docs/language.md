...
Application state callbacks use ordinary functions rather than controller objects. Android passes saved instance state through the generated Activity/native boundary; Linux stores a bounded `onSaveState() -> str` result atomically under an application-ID-keyed state path (overrideable with `FLUX_APP_STATE_PATH`) and calls `onRestoreState(str)` before `onStart` on the next launch. Invalid or oversized records are ignored. This is current-target process restoration, not yet a portable configuration-change or cross-platform state contract.

...
The common `onTap: fn() -> void` property is available on every built-in native element and supports the same typed functional state-transition form as other zero-argument UI events. Linux lowers taps through native `GtkGestureClick` release handling, while Android uses compiler-owned native touch-listener glue; neither backend exposes platform event/controller objects to Flux source.

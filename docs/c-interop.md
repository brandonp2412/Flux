...
Generated constants use compiler-prefixed names such as `flux__const_DEFAULT_LIMIT`, while callable exports use compiler ABI symbol names such as `flux__fn_parse`. The generated header is C++ compatible through an `extern "C"` block. Compiler regression coverage runs the generated header through Clang as C11, so source-level alias changes cannot quietly produce an invalid foreign header.

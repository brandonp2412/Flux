...
The native cache fingerprints sysroot contents as well as the active compiler/linker and target configuration. For CI evidence, `flux build ... --reproducibility <metadata>` writes deterministic `flux-reproducibility-v1` metadata containing the Flux version, build mode, generated-source hash, manifest and root-lock hashes, target, sysroot fingerprint, explicit compiler/SDK/runtime identities, and normalized `SOURCE_DATE_EPOCH`/`LC_ALL`/`TZ` inputs. Retain this metadata beside the binary; `flux verify-reproducibility <metadata-a> <metadata-b>` compares every recorded field, including artifact hashes, and fails on any mismatch without depending on host-specific paths.

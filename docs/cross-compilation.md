...
The ordinary build cache still keys an explicit sysroot by path and is optimized for local reuse, not cross-machine attestation. Release/CI builds that need a portable verification contract should use `--write-reproducibility` / `--verify-reproducibility`; cross-target or explicit-sysroot records require an immutable `FLUX_SDK_ID` instead of treating the path as an SDK identity. See [`reproducible-builds.md`](reproducible-builds.md).

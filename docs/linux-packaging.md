...
Reproducible release infrastructure should still pin the Flux compiler revision and native toolchain, isolate or pin environment inputs, and retain the exact `flux.toml` shipped in the bundle. `flux build ... --reproducibility <metadata>` records the generated-source, manifest, root-lock, sysroot, compiler/linker, SDK/runtime, target, and normalized environment identities needed to verify that contract across machines; `flux verify-reproducibility` performs the strict comparison. For detached native debug information and crash address resolution, see [`debugging.md`](debugging.md).

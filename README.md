...
The current LSP slice publishes parse/type diagnostics for standalone open buffers using the same source spans, labels, and fix metadata as the compiler, serves canonical whole-document formatting, turns machine-applicable compiler fixes into editor quick-fix workspace edits, and provides safe declaration/unambiguous-symbol hover type information. Workspace/import overlay analysis, completion, scope-aware navigation/hover, and highlighting remain later LSP milestones.

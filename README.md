...
- concrete no-generics list syntax with homogeneous `T[]` literals, checked positive/negative indexing, Python-style zero-copy slicing, and filtered list comprehensions; the bootstrap currently keeps list values immutable/local until ownership-safe escaping storage is implemented;

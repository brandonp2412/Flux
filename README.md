...
The current LSP slice publishes parse/type diagnostics for standalone open buffers using the same source spans, labels, and fix metadata as the compiler, serves canonical whole-document formatting, turns machine-applicable compiler fixes into editor quick-fix workspace edits, provides safe declaration/unambiguous-symbol hover type information, and emits full-document semantic highlighting for lexical and typed symbol categories. Workspace/import overlay analysis, completion, and scope-aware navigation/hover remain later LSP milestones.

...
- Rust/C++-style namespace qualification with canonical `::` syntax: enum construction such as `Outcome::Ok(42)`, interface capabilities such as `Storage::load(...)`, and compiler-owned platform APIs such as `android::vibrate(...)`; `.` remains value/field access while legacy namespace-dot calls are accepted during bootstrap and canonicalized by the formatter;

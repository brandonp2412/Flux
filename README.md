...
- compiler tests and runnable native examples, including nested structs, struct destructuring, zero-cost type aliases, folded constants, payload enums, exhaustive matching, named/default parameters, higher-order functions, explicit mutation/`while`, flat-grid UI syntax, typed built-in UI properties, parameterized view composition, and native GTK4/Wayland Text/Button/TextInput/Toggle/Radio controls with typed callback dispatch, live text-change/submit callbacks, hover/leave events, button keyboard shortcuts, autofocus, password masking, maximum input length, tooltips, and accessible labels/descriptions.

...
fn main() -> i64 {{
    ...
    print(net.localAddress(socket, inspect))
    print(net.close(socket))
    return 0
}}
"#
    ...
    assert!(generated.contains("flux__net_local_address("));
    assert!(generated.contains("getpeername("));
    assert!(generated.contains("getsockname("));
    ...
    assert_eq!(lines[4], "127.0.0.1");
    assert!(lines[5].parse::<u16>().is_ok_and(|port| port > 0));
    assert_eq!(&lines[6..], &["nil", "nil"]);
    ...
    let local_callback_error = check_source(
        "fn inspect(_host: str) -> void {\n}\nfn main() -> i64 {\n    print(net.localAddress(1, inspect))\n    return 0\n}\n",
    )
    .expect_err("local address callback shape must be exact");
    assert!(
        local_callback_error
            .message
            .contains("net.localAddress callback")
    );
    let local_socket_error = check_source(
        "fn inspect(_host: str, _port: i64) -> void {\n}\nfn main() -> i64 {\n    print(net.localAddress(\"bad\", inspect))\n    return 0\n}\n",
    )
    .expect_err("local address socket must be an i64 handle");
    assert!(
        local_socket_error
            .message
            .contains("net.localAddress socket")
            && local_socket_error.message.contains("expected i64")
    );
...
"#;
    ...
    assert!(!unused_generated.contains("flux__net_local_address("));
    let _ = fs::remove_dir_all(&root);
    drop(listener);
}
...
"#;
    ...
    let port = listener.local_addr().unwrap().port();
    let root = std::env::temp_dir().join(format!("flux-http-send-request-{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
...
"#;
    ...
    .expect_err("oversized HTTP response body cap must fail");
    assert!(
        invalid_limit
            .message
            .contains("http.receiveResponseWithTextBody maxBodyBytes must be between 0 and 65536")
    );
    ...
    let unused_generated =
        compile_to_c(&unused).expect("dead HTTP response body helper should tree-shake");
    assert!(!unused_generated.contains("flux__net_http_receive_response_with_text_body("));
    ...
    let root = std::env::temp_dir().join(format!("flux-http-client-{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).expect("HTTP fixture should be writable");
    ...
    fs::write(
        &source_path,
        format!(
            "fn response(socket: i64, version: str, status: i64, reason: str) -> void {{\n    print(socket >= 0)\n    print(version)\n    print(status)\n    print(reason)\n}}\nfn header(socket: i64, name: str, value: str) -> void {{\n    print(socket >= 0)\n    print(name)\n    print(value)\n}}\nfn body(socket: i64, value: str) -> void {{\n    print(socket >= 0)\n    print(value)\n}}\nfn main() -> i64 {{\n    let (socket, connectError) = net.tcpConnect(\"127.0.0.1\", {port})\n    print(connectError)\n    print(http.sendTextRequest(socket, \"POST\", \"/echo\", \"127.0.0.1:{port}\", \"text/plain\", \"ping\"))\n    let (received, receiveError) = http.receiveResponseWithTextBody(socket, 4096, 1024, response, header, body)\n    print(received > 0)\n    print(receiveError)\n    print(net.close(socket))\n    return 0\n}}\n"
        ),
    )
    .expect("HTTP client source should be writable");
    ...
    let built = Command::new(env!("CARGO_BIN_EXE_flux"))
        ...
        .arg(&binary)
        .output()
        .expect("HTTP client binary should build");
    assert!(
        built.status.success(),
        "HTTP client fixture build failed: {}",
        String::from_utf8_lossy(&built.stderr)
    );
    let run = Command::new(&binary)
        .output()
        .expect("HTTP client binary should run");
...
fn udp_scatter_gather_text_send_is_typed_tree_shaken_and_runnable() {
    ...
    print(bindError)
    print(net.sendTextToParts(socket, "127.0.0.1", {peer_port}, ["hello", "", " ", "world"]))
    print(net.close(socket))
    return 0
}}
"#
    );
    check_source(&source).expect("UDP scatter/gather text send should typecheck");
    ...
    let root = std::env::temp_dir().join(format!("flux-udp-scatter-gather-{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).expect("UDP scatter/gather fixture should be writable");
    let source_path = root.join("udp_scatter.flux");
    fs::write(&source_path, &source).expect("UDP scatter/gather source should be writable");
    let binary = root.join("udp_scatter");
    let built = Command::new(env!("CARGO_BIN_EXE_flux"))
        .arg("build")
        .arg(&source_path)
        .arg("-o")
        .arg(&binary)
        .output()
        .expect("UDP scatter/gather binary should build");
    assert!(
        built.status.success(),
        "UDP scatter/gather build failed: {}",
        String::from_utf8_lossy(&built.stderr)
    );
    ...
    let server = thread::spawn(move || {
        ...
        let (length, _) = peer
            .recv_from(&mut request)
            .expect("Flux UDP scatter/gather datagram should arrive");
        assert_eq!(length, 11);
        assert_eq!(&request[..length], b"hello world");
    });
    let run = Command::new(&binary)
        .output()
        .expect("UDP scatter/gather binary should run");
    server
        .join()
        .expect("UDP scatter/gather peer should finish");
    assert!(run.status.success());
    assert_eq!(String::from_utf8_lossy(&run.stdout), "nil\nnil\nnil\n");

    let port_error = check_source(
        "fn main() -> i64 {\n    print(net.sendTextToParts(1, \"127.0.0.1\", 0, [\"nope\"]))\n    return 0\n}\n",
    )
    .expect_err("constant UDP scatter/gather peer port zero must fail statically");
    assert!(
        port_error
            .message
            .contains("net.sendTextToParts port must be between 1 and 65535")
    );
    let wrong_parts = check_source(
        "fn main() -> i64 {\n    print(net.sendTextToParts(1, \"127.0.0.1\", 9999, [1, 2]))\n    return 0\n}\n",
    )
    .expect_err("UDP scatter/gather parts must be strings");
    assert!(wrong_parts.message.contains("net.sendTextToParts parts"));

    let unused = r#"
fn hidden(socket: i64) -> void {
    print(net.sendTextToParts(socket, "127.0.0.1", 9999, ["hidden", "payload"]))
}
fn main() -> i64 {
    return 0
}
"#;
    let unused_generated =
        compile_to_c(unused).expect("dead UDP scatter/gather send should tree-shake");
    assert!(!unused_generated.contains("flux__net_send_text_to_parts("));
    assert!(!unused_generated.contains("#include <sys/uio.h>"));
    let _ = fs::remove_dir_all(&root);
}
...
#[test]
fn eliminates_complementary_same_binding_boolean_comparisons() {
    let source = r#"
fn observe(value: bool) -> bool {
    print(value)
    return value
...
fn equalComplement(value: bool) -> bool {
    return value == !value
}

fn complementEqual(value: bool) -> bool {
    return !value == value
}

fn notEqualComplement(value: bool) -> bool {
    return value != !value
}

fn complementNotEqual(value: bool) -> bool {
    return !value != value
}

fn equalNegated(value: bool) -> bool {
    return !value == !value
}

fn notEqualNegated(value: bool) -> bool {
    return !value != !value
}
...
fn main() -> i64 {
    print(equalComplement(true))
    print(complementEqual(false))
    print(notEqualComplement(true))
    print(complementNotEqual(false))
    print(equalNegated(true))
    print(notEqualNegated(false))
    print(effectful(true))
    return 0
}
"#;

    check_source(source).expect("same-binding boolean comparison proofs should typecheck");
    let generated =
        compile_to_c(source).expect("same-binding boolean comparison proofs should lower natively");
    assert_eq!(generated.matches("return false;").count(), 3);
    assert_eq!(generated.matches("return true;").count(), 3);
    assert_eq!(
        generated
            .matches("flux__fn_observe(flux__local_value)")
            .count(),
        2,
        "effectful comparison operands must retain both source evaluations",
    );
...
"#;
    let ui_generated =
        compile_to_c(ui).expect("UI same-binding boolean comparison proofs should share lowering");
    assert!(!ui_generated.contains("flux__ui_state_active == (!(flux__ui_state_active))"));
    assert!(!ui_generated.contains("(!(flux__ui_state_active)) != flux__ui_state_active"));
    assert!(!ui_generated.contains("(!(flux__ui_state_active)) == (!(flux__ui_state_active))"));
    assert!(!ui_generated.contains("(!(flux__ui_state_active)) != (!(flux__ui_state_active))"));
...
"#;
    ...
    check_source(source).expect("portable confirm dialog should typecheck");
...
fn unused() -> void {
    dialog.confirm("Unused", "Hidden", confirmed)
}
view Screen {
    grid columns: 1fr
    grid rows: auto
...
app Screen
"#;
    let tree_shaken = compile_to_c(unused).expect("unreachable confirm code should tree-shake");
    assert!(!tree_shaken.contains("flux__dialog_confirm"));
...
"#;
    let errors =
        check_source_all(invalid).expect_err("dialog confirm arguments must be statically typed");
    assert!(errors.iter().any(|error| {
        error.message.contains("dialog.confirm title") && error.message.contains("expected str")
    }));
    assert!(errors.iter().any(|error| {
        error.message.contains("dialog.confirm message") && error.message.contains("expected str")
    }));
    assert!(errors.iter().any(|error| {
        error.message.contains("dialog.confirm callback")
            && error.message.contains("expected fn() -> void")
    }));
    assert!(errors.iter().any(|error| {
...
"#,
    ...
    assert!(generated.contains("\"density\", \"F\""));
...
"##,
    ...
    assert!(generated.contains("INT64_C(18) * flux__ui_density"));
    assert!(generated.contains("INT64_C(6) * flux__ui_density"));
    assert!(generated.contains("setMinimumWidth"));
    assert!(generated.contains("styleView"));
    assert!(generated.contains("INT64_C(16) * flux__ui_density"));
    assert!(generated.contains("INT64_C(2) * flux__ui_density"));
    ...
    assert!(generated.contains("INT64_C(1) * flux__ui_density"));
    assert!(generated.contains("INT64_C(3) * flux__ui_density"));
    assert!(generated.contains("INT64_C(4) * flux__ui_density"));
    assert!(generated.contains("INT64_C(5) * flux__ui_density"));
    assert!(generated.contains("INT64_C(7) * flux__ui_density"));
    assert!(generated.contains("INT64_C(-2) * flux__ui_density"));
...
app DynamicBackground
"#;

    check_source(source).expect("dynamic Android backgroundColor should typecheck");
    let database = fluxc::semantic::SemanticDatabase::analyze(source, SourceId::UNKNOWN)
        .expect("dynamic Android backgroundColor should analyze");
    let android = fluxc::codegen::emit_c_for_target_with_source_paths(
        database.program(),
        database.signatures(),
        &std::collections::HashMap::new(),
    ...
    let panel_id = android_stable_view_id("DynamicBackground", "panel");
...
app StaticBackground
"#;
    let static_database =
        fluxc::semantic::SemanticDatabase::analyze(static_source, SourceId::UNKNOWN)
            .expect("static Android backgroundColor should analyze");
    let static_android = fluxc::codegen::emit_c_for_target_with_source_paths(
        static_database.program(),
        static_database.signatures(),
        &std::collections::HashMap::new(),
    ...
    .expect("static Android backgroundColor should lower");
    assert!(!static_android.contains("styleViewBackground"));

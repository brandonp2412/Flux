...
fn render_lockfile(manifest: &PackageManifest) -> Result<String, Vec<Diagnostic>> {
    ...
    let root = manifest
        ...
        .to_path_buf();
    active.insert(root);
    collect_locked_dependencies(manifest, "", &mut active, &mut entries)?;
...
fn collect_locked_dependencies(
    ...
    for (name, dependency) in &manifest.dependencies {
        ...
        match dependency {
            PackageDependency::Registry { requirement } => entries.push(LockedDependency {
                id,
                package: name.clone(),
                version: None,
                source: format!("registry:{requirement}"),
            }),
            PackageDependency::Git { url, rev } => entries.push(LockedDependency {
                id,
                package: name.clone(),
                version: None,
            ...
            PackageDependency::Path { path, requirement } => {
                ...
                if let Some(requirement) = requirement {
                    let Some(version) = dependency_manifest.version.as_deref() else {
                        return Err(vec![Diagnostic::global(
                            DiagnosticStage::Parse,
                            format!(
                                "package dependency '{name}' requires version '{requirement}', but path package has no [package].version"
                            ),
                        )]);
                    };
                    if !semver_requirement_matches(requirement, version) {
                        return Err(vec![Diagnostic::global(
                            DiagnosticStage::Parse,
                            format!(
                                "package dependency '{name}' requires version '{requirement}', but path package '{}' is version '{version}'",
                ...
                if !active.insert(dependency_root.clone()) {
                    return Err(vec![Diagnostic::global(
                        DiagnosticStage::Parse,
                        format!(
                            "cyclic path dependency detected while locking '{}'",
                            dependency_manifest.name
                        ),
                    )]);
                }
                collect_locked_dependencies(&dependency_manifest, &id, active, entries)?;

...
fn is_unit_enum_constructor(expr: &Expr, signatures: &Signatures) -> bool {
    ...
    args.is_empty()
        ...
        && signatures
            .enum_type(namespace)
            .and_then(|definition| definition.variant(name))
        ...
        ExprKind::List(items) | ExprKind::Set(items) => {
            ...
            let item_element_type = |item: &Expr| -> Result<Type, Diagnostic> {
                ...
                if is_set
                    ...
                    && !is_unit_enum_constructor(item, signatures)
                {
                    return Err(diag(
                        item.span,
                        "set literal elements must currently be compile-time primitive values or unit enum constructors",
                    ));
                }
                match &item.kind {
                    ExprKind::ListSpread {
...
fn check_qualified_call(
        ...
        match name.as_str() {
            "length" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("str.length expects 1 argument, got {}", args.len()),
                    ));
                }
                let value = type_of_expr(&args[0], env, signatures)?;
                require_type(args[0].span, &Type::Str, &value, "str.length value")?;
        ...
        match name.as_str() {
            ...
            "resumed" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("tls.resumed expects 1 argument, got {}", args.len()),
                    ));
                }
                let session = type_of_expr(&args[0], env, signatures)?;
                require_type(args[0].span, &Type::I64, &session, "tls.resumed session")?;
            ...
            "pending" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("tls.pending expects 1 argument, got {}", args.len()),
                    ));
                }
                let session = type_of_expr(&args[0], env, signatures)?;
                require_type(args[0].span, &Type::I64, &session, "tls.pending session")?;
                return Ok(vec![Type::I64, Type::Error]);
            }
            "read" => {
                if args.len() != 3 {
        ...
        match name.as_str() {
            ...
            "acceptManyTimeout" | "tcpAcceptManyWithTimeout" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.acceptManyTimeout expects 4 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &listener,
                    "net.acceptManyTimeout listener",
                )?;
                let max_count = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_count,
                    "net.acceptManyTimeout maxCount",
                )?;
                if matches!(
                    constant_primitive_value(&args[1], signatures),
                    Some(ConstantValue::I64(value)) if value < 1 || value > i32::MAX as i64
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.acceptManyTimeout maxCount must be between 1 and 2147483647",
                    ));
                }
                let timeout = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.acceptManyTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.acceptManyTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[3], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.acceptManyTimeout callback",
                )?;
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            ...
            "waitAny" => {
                    ...
                    && items.len() > MAX_SOCKET_READINESS_DESCRIPTORS
                {
                    return Err(diag(
                        args[0].span,
                        &format!(
                            "net.waitAny sockets must contain at most {MAX_SOCKET_READINESS_DESCRIPTORS} handles"
                        ),
                    ));
                }
                let timeout = type_of_expr(&args[1], env, signatures)?;
                ...
                if matches!(
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.waitAny timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            "sendTextWithTimeout" => {
                if args.len() != 3 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.sendTextWithTimeout expects 3 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.sendTextWithTimeout socket",
                )?;
                ...
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.sendTextWithTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
            ...
            "sendBytesProgress" => {
                if args.len() != 3 {
                    return Err(diag(
                        span,
                        &format!("net.writeBytesFrom expects 3 arguments, got {}", args.len()),
                    ));
                }
                let handle = type_of_expr(&args[0], env, signatures)?;
                require_type(
            ...
            "sendBytesProgressWithTimeout" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.writeBytesFromTimeout expects 4 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
            ...
            "sendTextPartsWithTimeout" => {
                if args.len() != 3 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.sendTextPartsWithTimeout expects 3 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::List(Box::new(Type::Str)),
                    &parts,
                    "net.sendTextPartsWithTimeout parts",
                )?;
                let timeout = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    args[2].span,
                    &Type::I64,
                    &timeout,
                    "net.sendTextPartsWithTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.sendTextPartsWithTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                return Ok(vec![Type::I64, Type::Error]);
            }
            ...
            "receiveTextManyWithTimeout" => {
                if args.len() != 5 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.receiveTextManyWithTimeout expects 5 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.receiveTextManyWithTimeout socket",
                )?;
                let max_bytes = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_bytes,
                    "net.receiveTextManyWithTimeout maxBytes",
                )?;
                if matches!(
                    constant_primitive_value(&args[1], signatures),
                    Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value)
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.receiveTextManyWithTimeout maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &max_count,
                    "net.receiveTextManyWithTimeout maxCount",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if value < 1 || value > i32::MAX as i64
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.receiveTextManyWithTimeout maxCount must be between 1 and 2147483647",
                    ));
                }
                let timeout = type_of_expr(&args[3], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.receiveTextManyWithTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[3], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[3].span,
                        "net.receiveTextManyWithTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[4], env, signatures)?);
                let expected = Type::Function {
                    params: vec![Type::I64, Type::Str],
                    returns: Vec::new(),
                };
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.receiveTextManyWithTimeout callback",
                )?;
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            "receiveText" => {
            ...
            "readBytesMany" | "receiveBytesMany" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!("net.readBytesMany expects 4 arguments, got {}", args.len()),
                    ));
                }
                let handle = type_of_expr(&args[0], env, signatures)?;
                require_type(
                ...
                if matches!(
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.readBytesMany maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                ...
                if matches!(
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.readBytesMany maxCount must be between 1 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[3], env, signatures)?);
                let expected = Type::Function {
            ...
            "readBytesManyTimeout" | "receiveBytesManyWithTimeout" => {
                if args.len() != 5 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.readBytesManyTimeout expects 5 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.readBytesManyTimeout socket",
                )?;
                let max_bytes = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_bytes,
                    "net.readBytesManyTimeout maxBytes",
                )?;
                if matches!(
                    constant_primitive_value(&args[1], signatures),
                    Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value)
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.readBytesManyTimeout maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &max_count,
                    "net.readBytesManyTimeout maxCount",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if !(1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.readBytesManyTimeout maxCount must be between 1 and 2147483647",
                    ));
                }
                let timeout = type_of_expr(&args[3], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.readBytesManyTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[3], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[3].span,
                        "net.readBytesManyTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[4], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.readBytesManyTimeout callback",
                )?;
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            "readBytesFromTimeout" | "receiveBytesFromWithTimeout" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.readBytesFromTimeout expects 4 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.readBytesFromTimeout socket",
                )?;
                let max_bytes = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_bytes,
                    "net.readBytesFromTimeout maxBytes",
                )?;
                if matches!(
                    constant_primitive_value(&args[1], signatures),
                    Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value)
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.readBytesFromTimeout maxBytes must be between 1 and 65536",
                    ));
                }
                let timeout = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.readBytesFromTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.readBytesFromTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[3], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.readBytesFromTimeout callback",
                )?;
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            "readBytesFromMany" | "receiveBytesFromMany" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.readBytesFromMany expects 4 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &max_bytes,
                    "net.readBytesFromMany maxBytes",
                )?;
                if matches!(constant_primitive_value(&args[1], signatures), Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value))
                {
                    return Err(diag(
                        args[1].span,
                        "net.readBytesFromMany maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &max_count,
                    "net.readBytesFromMany maxCount",
                )?;
                if matches!(constant_primitive_value(&args[2], signatures), Some(ConstantValue::I64(value)) if !(1..=i32::MAX as i64).contains(&value))
                {
                    return Err(diag(
                        args[2].span,
                        "net.readBytesFromMany maxCount must be between 1 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[3], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.readBytesFromMany callback",
                )?;
                return Ok(vec![Type::I64, Type::Error]);
            }
            "readBytesFromManyTimeout" | "receiveBytesFromManyWithTimeout" => {
                if args.len() != 5 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.readBytesFromManyTimeout expects 5 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.readBytesFromManyTimeout socket",
                )?;
                let max_bytes = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_bytes,
                    "net.readBytesFromManyTimeout maxBytes",
                )?;
                if matches!(constant_primitive_value(&args[1], signatures), Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value))
                {
                    return Err(diag(
                        args[1].span,
                        "net.readBytesFromManyTimeout maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &max_count,
                    "net.readBytesFromManyTimeout maxCount",
                )?;
                if matches!(constant_primitive_value(&args[2], signatures), Some(ConstantValue::I64(value)) if !(1..=i32::MAX as i64).contains(&value))
                {
                    return Err(diag(
                        args[2].span,
                        "net.readBytesFromManyTimeout maxCount must be between 1 and 2147483647",
                    ));
                }
                let timeout = type_of_expr(&args[3], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.readBytesFromManyTimeout timeoutMillis",
                )?;
                if matches!(constant_primitive_value(&args[3], signatures), Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value))
                {
                    return Err(diag(
                        args[3].span,
                        "net.readBytesFromManyTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[4], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.readBytesFromManyTimeout callback",
                )?;
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            "readBytesTimeout" | "receiveBytesWithTimeout" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.readBytesTimeout expects 4 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &max_bytes,
                    "net.readBytesTimeout maxBytes",
                )?;
                if matches!(constant_primitive_value(&args[1], signatures), Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value))
                {
                    return Err(diag(
                        args[1].span,
                        "net.readBytesTimeout maxBytes must be between 1 and 65536",
                    ));
                }
                let timeout = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.readBytesTimeout timeoutMillis",
                )?;
                if matches!(constant_primitive_value(&args[2], signatures), Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value))
                {
                    return Err(diag(
                        args[2].span,
                        "net.readBytesTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[3], env, signatures)?);
                let expected = Type::Function {
            ...
            "receiveTextFromMany" => {
                if args.len() != 4 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.receiveTextFromMany expects 4 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.receiveTextFromMany socket",
                )?;
                let max_bytes = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_bytes,
                    "net.receiveTextFromMany maxBytes",
                )?;
                if matches!(
                    constant_primitive_value(&args[1], signatures),
                    Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value)
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.receiveTextFromMany maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &max_count,
                    "net.receiveTextFromMany maxCount",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if value < 1 || value > i32::MAX as i64
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.receiveTextFromMany maxCount must be between 1 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[3], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.receiveTextFromMany callback",
                )?;
                return Ok(vec![Type::I64, Type::Error]);
            }
            "receiveTextFromManyWithTimeout" => {
                if args.len() != 5 {
                    return Err(diag(
                        span,
                        &format!(
                            "net.receiveTextFromManyWithTimeout expects 5 arguments, got {}",
                            args.len()
                        ),
                    ));
                }
                ...
                require_type(
                    ...
                    &Type::I64,
                    &handle,
                    "net.receiveTextFromManyWithTimeout socket",
                )?;
                let max_bytes = type_of_expr(&args[1], env, signatures)?;
                require_type(
                    args[1].span,
                    &Type::I64,
                    &max_bytes,
                    "net.receiveTextFromManyWithTimeout maxBytes",
                )?;
                if matches!(
                    constant_primitive_value(&args[1], signatures),
                    Some(ConstantValue::I64(value)) if !(1..=65536).contains(&value)
                    ...
                    return Err(diag(
                        args[1].span,
                        "net.receiveTextFromManyWithTimeout maxBytes must be between 1 and 65536",
                    ));
                }
                let max_count = type_of_expr(&args[2], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &max_count,
                    "net.receiveTextFromManyWithTimeout maxCount",
                )?;
                if matches!(
                    constant_primitive_value(&args[2], signatures),
                    Some(ConstantValue::I64(value)) if value < 1 || value > i32::MAX as i64
                    ...
                    return Err(diag(
                        args[2].span,
                        "net.receiveTextFromManyWithTimeout maxCount must be between 1 and 2147483647",
                    ));
                }
                let timeout = type_of_expr(&args[3], env, signatures)?;
                require_type(
                    ...
                    &Type::I64,
                    &timeout,
                    "net.receiveTextFromManyWithTimeout timeoutMillis",
                )?;
                if matches!(
                    constant_primitive_value(&args[3], signatures),
                    Some(ConstantValue::I64(value)) if !(-1..=i32::MAX as i64).contains(&value)
                    ...
                    return Err(diag(
                        args[3].span,
                        "net.receiveTextFromManyWithTimeout timeoutMillis must be -1 or between 0 and 2147483647",
                    ));
                }
                let callback = signatures.canonical_type(&type_of_expr(&args[4], env, signatures)?);
                let expected = Type::Function {
                ...
                require_type(
                    ...
                    &expected,
                    &callback,
                    "net.receiveTextFromManyWithTimeout callback",
                )?;
                return Ok(vec![Type::I64, Type::Bool, Type::Error]);
            }
            "receiveTextFrom" => {
        ...
        match name.as_str() {
            ...
            "receiveRequestWithBinaryBody" | "receiveRequestWithBinaryBodyTrailers" => {
                ...
                require_type(
                    ...
                    &Type::I64,
                    &socket,
                    "http.receiveRequestWithBinaryBody socket",
                )?;
                for (index, label, minimum) in [
                    (1usize, "maxHeadBytes", 1i64),
                    (2usize, "maxBodyBytes", 0i64),
                    ...
                    require_type(
                        ...
                        &Type::I64,
                        &limit,
                        &format!("http.receiveRequestWithBinaryBody {label}"),
                    )?;
                    if matches!(
                        constant_primitive_value(&args[index], signatures),
                        Some(ConstantValue::I64(value)) if value < minimum || value > 65536
                        ...
                        return Err(diag(
                            args[index].span,
                            &format!(
                                "http.receiveRequestWithBinaryBody {label} must be between {minimum} and 65536"
                            ),
                        ));
                    }
                }
                ...
                require_type(
                    ...
                    &expected_request_callback,
                    &request_callback,
                    "http.receiveRequestWithBinaryBody requestCallback",
                )?;
                let header_callback =
                    signatures.canonical_type(&type_of_expr(&args[4], env, signatures)?);
                let expected_header_callback = Type::Function {
                ...
                require_type(
                    ...
                    &expected_header_callback,
                    &header_callback,
                    "http.receiveRequestWithBinaryBody headerCallback",
                )?;
                let body_callback =
                    signatures.canonical_type(&type_of_expr(&args[5], env, signatures)?);
                let expected_body_callback = Type::Function {
            ...
            "receiveResponseWithBinaryBody" | "receiveResponseWithBinaryBodyTrailers" => {
                ...
                require_type(
                    ...
                    &Type::I64,
                    &socket,
                    "http.receiveResponseWithBinaryBody socket",
                )?;
                for (index, label, minimum) in [
                    (1usize, "maxHeadBytes", 1i64),
                    (2usize, "maxBodyBytes", 0i64),
                    ...
                    require_type(
                        ...
                        &Type::I64,
                        &limit,
                        &format!("http.receiveResponseWithBinaryBody {label}"),
                    )?;
                    if matches!(
                        constant_primitive_value(&args[index], signatures),
                        Some(ConstantValue::I64(value)) if value < minimum || value > 65536
                        ...
                        return Err(diag(
                            args[index].span,
                            &format!(
                                "http.receiveResponseWithBinaryBody {label} must be between {minimum} and 65536"
                            ),
                        ));
                    }
                }
                ...
                require_type(
                    ...
                    &expected_response_callback,
                    &response_callback,
                    "http.receiveResponseWithBinaryBody responseCallback",
                )?;
                let header_callback =
                    signatures.canonical_type(&type_of_expr(&args[4], env, signatures)?);
                let expected_header_callback = Type::Function {
                ...
                require_type(
                    ...
                    &expected_header_callback,
                    &header_callback,
                    "http.receiveResponseWithBinaryBody headerCallback",
                )?;
                let body_callback =
                    signatures.canonical_type(&type_of_expr(&args[5], env, signatures)?);
                let expected_body_callback = Type::Function {
        ...
        match name.as_str() {
            "validate" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("json.validate expects 1 argument, got {}", args.len()),
                    ));
                }
                let value = type_of_expr(&args[0], env, signatures)?;
                require_type(args[0].span, &Type::Str, &value, "json.validate value")?;
                return Ok(vec![Type::Error]);
            }
            "parse" => {
                if args.len() != 2 {
        ...
        match name.as_str() {
            ...
            "done" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("worker.done expects 1 argument, got {}", args.len()),
                    ));
                }
                let actual = type_of_expr(&args[0], env, signatures)?;
            ...
            "failure" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("worker.failure expects 1 argument, got {}", args.len()),
                    ));
                }
                let actual = type_of_expr(&args[0], env, signatures)?;
                require_type(args[0].span, &Type::I64, &actual, "worker.failure handle")?;
                return Ok(vec![Type::Error]);
            }
            "waitAny" | "joinAny" => {
                if args.len() != 1 {
            ...
            "cancel" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("worker.cancel expects 1 argument, got {}", args.len()),
                    ));
                }
                let actual = type_of_expr(&args[0], env, signatures)?;
            ...
            "cancelled" => {
                if !args.is_empty() {
                    return Err(diag(
                        span,
                        &format!("worker.cancelled expects 0 arguments, got {}", args.len()),
                    ));
                }
                return Ok(vec![Type::Bool]);
            }
    ...
    if namespace == "file" {
        if !named_args.is_empty() {
            return Err(diag(
                span,
                &format!("file.{name} accepts positional arguments only"),
            ));
        }
        match name.as_str() {
            ...
            | "allocatedSize" | "remove" | "sync" | "syncData" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("file.{name} expects 1 argument, got {}", args.len()),
                    ));
                }
            ...
            _ => {
                return Err(diag(
                    *name_span,
                    &format!("file module has no function '{name}'"),
                ));
            }
        }
    }
    ...
    if namespace == "directory" {
        if !named_args.is_empty() {
            return Err(diag(
                span,
                &format!("directory.{name} accepts positional arguments only"),
            ));
        }
        match name.as_str() {
            "list" => {
            ...
            | "create" | "createAll" | "remove" | "removeAll" | "sync" => {
                if args.len() != 1 {
                    return Err(diag(
                        span,
                        &format!("directory.{name} expects 1 argument, got {}", args.len()),
                    ));
                }
                let path_type = type_of_expr(&args[0], env, signatures)?;
                require_type(
            ...
            _ => {
                return Err(diag(
                    *name_span,
                    &format!("directory module has no function '{name}'"),
                ));
            }
        }
    }

...
fn emit_runtime_prelude(
    ...
    if uses_android_keep_screen_on && uses_android_generated_ui {
        ...
        out.push_str("    jobject window = NULL; jclass window_class = NULL;\n");
        out.push_str("    if (activity_class == NULL) goto done;\n");
        out.push_str("    jmethodID get_window = (*env)->GetMethodID(env, activity_class, \"getWindow\", \"()Landroid/view/Window;\");\n");
        out.push_str("    if (get_window == NULL) goto done;\n");
        out.push_str("    window = (*env)->CallObjectMethod(env, activity, get_window);\n");
        out.push_str("    if ((*env)->ExceptionCheck(env) || window == NULL) goto done;\n");
        out.push_str("    window_class = (*env)->GetObjectClass(env, window);\n");
        out.push_str("    if (window_class == NULL) goto done;\n");
        out.push_str("    jmethodID set_flags = (*env)->GetMethodID(env, window_class, \"setFlags\", \"(II)V\");\n");
        out.push_str("    if (set_flags != NULL) (*env)->CallVoidMethod(env, window, set_flags, (jint)(enabled ? AWINDOW_FLAG_KEEP_SCREEN_ON : 0), (jint)AWINDOW_FLAG_KEEP_SCREEN_ON);\n");
        out.push_str("done:\n");
        out.push_str("    if ((*env)->ExceptionCheck(env)) (*env)->ExceptionClear(env);\n");
        out.push_str("    if (window_class != NULL) (*env)->DeleteLocalRef(env, window_class);\n");
        out.push_str("    if (window != NULL) (*env)->DeleteLocalRef(env, window);\n");
        out.push_str(
...
"#);
    ...
    for (size_t input = 0; input < length; input += 1) {
        ...
        if (input + 2 >= length) return "Form URL component has incomplete percent escape";
        unsigned char high = (unsigned char)value[input + 1];
        unsigned char low = (unsigned char)value[input + 2];
        int high_value = high >= '0' && high <= '9' ? high - '0' : high >= 'a' && high <= 'f' ? high - 'a' + 10 : high >= 'A' && high <= 'F' ? high - 'A' + 10 : -1;
        int low_value = low >= '0' && low <= '9' ? low - '0' : low >= 'a' && low <= 'f' ? low - 'a' + 10 : low >= 'A' && low <= 'F' ? low - 'A' + 10 : -1;
        if (high_value < 0 || low_value < 0) return "Form URL component has invalid percent escape";
        unsigned char decoded_byte = (unsigned char)((high_value << 4) | low_value);
        if (decoded_byte == 0) return "Form URL component cannot decode to NUL";
        decoded[output++] = (char)decoded_byte;
        input += 2;
    }
    decoded[output] = '\0';
...
"#);
    ...
    for (size_t input = 0; input < length; input += 1) {
        unsigned char byte = (unsigned char)value[input];
        bool plain = (byte >= 'a' && byte <= 'z') || (byte >= 'A' && byte <= 'Z') || (byte >= '0' && byte <= '9') || byte == '*' || byte == '-' || byte == '.' || byte == '_';
        encoded_length += plain || byte == ' ' ? 1 : 3;
        if (encoded_length > 65536) return "Encoded form URL component exceeds 65536 bytes";
    }
    char encoded[65537];
    size_t output = 0;
    for (size_t input = 0; input < length; input += 1) {
...
"#);
    ...
    if runtime_usage.contains("flux__net_http_receive_request_head_with_headers(") {
        out.push_str("static inline struct flux__net_i64_error flux__net_http_receive_request_head_with_headers(int64_t socket_handle, int64_t max_bytes, void (*request_callback)(int64_t, const char *, const char *, const char *), void (*header_callback)(int64_t, const char *, const char *)) { if (socket_handle < 0 || socket_handle > INT_MAX) return flux__net_result(-1, \"invalid socket handle\"); if (max_bytes < 1 || max_bytes > 65536) return flux__net_result(-1, \"receiveRequestHeadWithHeaders maxBytes must be between 1 and 65536\"); int socket_type = 0; socklen_t type_length = sizeof(socket_type); if (getsockopt((int)socket_handle, SOL_SOCKET, SO_TYPE, &socket_type, &type_length) != 0) return flux__net_result(-1, \"failed to inspect socket type\"); if (socket_type != SOCK_STREAM) return flux__net_result(-1, \"HTTP request requires a TCP socket\"); char buffer[65537]; size_t length = 0; bool complete = false; while (length < (size_t)max_bytes) { ssize_t received; do { received = recv((int)socket_handle, buffer + length, 1, 0); } while (received < 0 && errno == EINTR); if (received < 0) return flux__net_result(-1, \"failed to receive HTTP request head\"); if (received == 0) return flux__net_result(-1, \"connection closed before HTTP request head completed\"); if (buffer[length] == '\\0') return flux__net_result(-1, \"HTTP request head contains a NUL byte\"); length += 1; if (length >= 4 && memcmp(buffer + length - 4, \"\\r\\n\\r\\n\", 4) == 0) { complete = true; break; } } if (!complete) return flux__net_result(-1, \"HTTP request head exceeds maxBytes\"); buffer[length] = '\\0'; char *line_end = strstr(buffer, \"\\r\\n\"); if (line_end == NULL) return flux__net_result(-1, \"malformed HTTP request line\"); *line_end = '\\0'; char *method = buffer; char *first_space = strchr(method, ' '); if (first_space == NULL || first_space == method) return flux__net_result(-1, \"malformed HTTP request line\"); *first_space = '\\0'; char *target = first_space + 1; char *second_space = strchr(target, ' '); if (second_space == NULL || second_space == target) return flux__net_result(-1, \"malformed HTTP request line\"); *second_space = '\\0'; char *version = second_space + 1; if (version[0] == '\\0' || strchr(version, ' ') != NULL) return flux__net_result(-1, \"malformed HTTP request line\"); for (const unsigned char *cursor = (const unsigned char *)method; *cursor != '\\0'; cursor += 1) if (*cursor <= 0x20 || *cursor == 0x7f) return flux__net_result(-1, \"invalid HTTP method\"); for (const unsigned char *cursor = (const unsigned char *)target; *cursor != '\\0'; cursor += 1) if (*cursor <= 0x20 || *cursor == 0x7f) return flux__net_result(-1, \"invalid HTTP request target\"); if (strcmp(version, \"HTTP/1.1\") != 0 && strcmp(version, \"HTTP/1.0\") != 0) return flux__net_result(-1, \"unsupported HTTP version\"); char *headers = line_end + 2; char *cursor = headers; while (!(cursor[0] == '\\r' && cursor[1] == '\\n')) { char *next = strstr(cursor, \"\\r\\n\"); if (next == NULL) return flux__net_result(-1, \"malformed HTTP header line\"); if (cursor[0] == ' ' || cursor[0] == '\\t') return flux__net_result(-1, \"folded HTTP headers are not supported\"); char *colon = memchr(cursor, ':', (size_t)(next - cursor)); if (colon == NULL || colon == cursor) return flux__net_result(-1, \"malformed HTTP header field\"); for (const unsigned char *part = (const unsigned char *)cursor; part < (const unsigned char *)colon; part += 1) { bool token = (*part >= '0' && *part <= '9') || (*part >= 'A' && *part <= 'Z') || (*part >= 'a' && *part <= 'z') || strchr(\"!#$%&'*+-.^_`|~\", *part) != NULL; if (!token) return flux__net_result(-1, \"invalid HTTP header name\"); } for (const unsigned char *part = (const unsigned char *)(colon + 1); part < (const unsigned char *)next; part += 1) if ((*part < 0x20 && *part != '\\t') || *part == 0x7f) return flux__net_result(-1, \"invalid HTTP header value\"); cursor = next + 2; } request_callback(socket_handle, method, target, version); cursor = headers; while (!(cursor[0] == '\\r' && cursor[1] == '\\n')) { char *next = strstr(cursor, \"\\r\\n\"); char *colon = memchr(cursor, ':', (size_t)(next - cursor)); *colon = '\\0'; char *value = colon + 1; while (*value == ' ' || *value == '\\t') value += 1; char *value_end = next; while (value_end > value && (value_end[-1] == ' ' || value_end[-1] == '\\t')) value_end -= 1; *value_end = '\\0'; header_callback(socket_handle, cursor, value); cursor = next + 2; } return flux__net_result((int64_t)length, NULL); }\n");
    }
    ...
    if (max_body_bytes < 0 || max_body_bytes > 65536) return flux__net_result(-1, "receiveRequestWithTextBody maxBodyBytes must be between 0 and 65536");
    int socket_type = 0;
    ...
    if (socket_type != SOCK_STREAM) return flux__net_result(-1, "HTTP request requires a TCP socket");
    char head[65537];
    ...
    if (!complete) return flux__net_result(-1, "HTTP request head exceeds maxHeadBytes");
    head[head_length] = '\0';
    char *line_end = strstr(head, "\r\n");
    if (line_end == NULL) return flux__net_result(-1, "malformed HTTP request line");
    *line_end = '\0';
    char *method = head;
    char *first_space = strchr(method, ' ');
    if (first_space == NULL || first_space == method) return flux__net_result(-1, "malformed HTTP request line");
    *first_space = '\0';
    char *target = first_space + 1;
    char *second_space = strchr(target, ' ');
    if (second_space == NULL || second_space == target) return flux__net_result(-1, "malformed HTTP request line");
    *second_space = '\0';
    char *version = second_space + 1;
    if (version[0] == '\0' || strchr(version, ' ') != NULL) return flux__net_result(-1, "malformed HTTP request line");
    for (const unsigned char *part = (const unsigned char *)method; *part != '\0'; part += 1) if (*part <= 0x20 || *part == 0x7f) return flux__net_result(-1, "invalid HTTP method");
    for (const unsigned char *part = (const unsigned char *)target; *part != '\0'; part += 1) if (*part <= 0x20 || *part == 0x7f) return flux__net_result(-1, "invalid HTTP request target");
    ...
    while (!(cursor[0] == '\r' && cursor[1] == '\n')) {
        ...
        if (name_length == 17 && strncasecmp(cursor, "Transfer-Encoding", 17) == 0) {
            ...
            if (strcmp(version, "HTTP/1.1") != 0) return flux__net_result(-1, "chunked request bodies require HTTP/1.1");
            chunked = true;
        }
    ...
    if (chunked) {
        for (;;) {
            ...
            for (char *digit = chunk_line; *digit != '\0'; digit += 1) {
                ...
                if (chunk_size > remaining_capacity / 16u) return flux__net_result(-1, "HTTP request body exceeds maxBodyBytes");
    ...
    body[body_length] = '\0';
    request_callback(socket_handle, method, target, version);
    cursor = headers;
    while (!(cursor[0] == '\r' && cursor[1] == '\n')) {
        char *next = strstr(cursor, "\r\n");
        char *colon = memchr(cursor, ':', (size_t)(next - cursor));
...
"#);
    ...
    if runtime_usage.contains("flux__net_http_receive_response_head_with_headers(") {
        out.push_str("static inline struct flux__net_i64_error flux__net_http_receive_response_head_with_headers(int64_t socket_handle, int64_t max_bytes, void (*response_callback)(int64_t, const char *, int64_t, const char *), void (*header_callback)(int64_t, const char *, const char *)) { if (socket_handle < 0 || socket_handle > INT_MAX) return flux__net_result(-1, \"invalid socket handle\"); if (max_bytes < 1 || max_bytes > 65536) return flux__net_result(-1, \"receiveResponseHeadWithHeaders maxBytes must be between 1 and 65536\"); int socket_type = 0; socklen_t type_length = sizeof(socket_type); if (getsockopt((int)socket_handle, SOL_SOCKET, SO_TYPE, &socket_type, &type_length) != 0) return flux__net_result(-1, \"failed to inspect socket type\"); if (socket_type != SOCK_STREAM) return flux__net_result(-1, \"HTTP response requires a TCP socket\"); char buffer[65537]; size_t length = 0; bool complete = false; while (length < (size_t)max_bytes) { ssize_t received; do { received = recv((int)socket_handle, buffer + length, 1, 0); } while (received < 0 && errno == EINTR); if (received < 0) return flux__net_result(-1, \"failed to receive HTTP response head\"); if (received == 0) return flux__net_result(-1, \"connection closed before HTTP response head completed\"); if (buffer[length] == '\\0') return flux__net_result(-1, \"HTTP response head contains a NUL byte\"); length += 1; if (length >= 4 && memcmp(buffer + length - 4, \"\\r\\n\\r\\n\", 4) == 0) { complete = true; break; } } if (!complete) return flux__net_result(-1, \"HTTP response head exceeds maxBytes\"); buffer[length] = '\\0'; char *line_end = strstr(buffer, \"\\r\\n\"); if (line_end == NULL) return flux__net_result(-1, \"malformed HTTP status line\"); *line_end = '\\0'; char *version = buffer; char *first_space = strchr(version, ' '); if (first_space == NULL || first_space == version) return flux__net_result(-1, \"malformed HTTP status line\"); *first_space = '\\0'; char *status_text = first_space + 1; char *second_space = strchr(status_text, ' '); if (second_space == NULL || second_space - status_text != 3) return flux__net_result(-1, \"malformed HTTP status line\"); *second_space = '\\0'; char *reason = second_space + 1; if (strcmp(version, \"HTTP/1.1\") != 0 && strcmp(version, \"HTTP/1.0\") != 0) return flux__net_result(-1, \"unsupported HTTP version\"); if (status_text[0] < '0' || status_text[0] > '9' || status_text[1] < '0' || status_text[1] > '9' || status_text[2] < '0' || status_text[2] > '9') return flux__net_result(-1, \"invalid HTTP status code\"); int64_t status = (int64_t)(status_text[0] - '0') * 100 + (int64_t)(status_text[1] - '0') * 10 + (int64_t)(status_text[2] - '0'); if (status < 100 || status > 599) return flux__net_result(-1, \"invalid HTTP status code\"); for (const unsigned char *part = (const unsigned char *)reason; *part != '\\0'; part += 1) if ((*part < 0x20 && *part != '\\t') || *part == 0x7f) return flux__net_result(-1, \"invalid HTTP reason phrase\"); char *headers = line_end + 2; char *cursor = headers; while (!(cursor[0] == '\\r' && cursor[1] == '\\n')) { char *next = strstr(cursor, \"\\r\\n\"); if (next == NULL) return flux__net_result(-1, \"malformed HTTP header line\"); if (cursor[0] == ' ' || cursor[0] == '\\t') return flux__net_result(-1, \"folded HTTP headers are not supported\"); char *colon = memchr(cursor, ':', (size_t)(next - cursor)); if (colon == NULL || colon == cursor) return flux__net_result(-1, \"malformed HTTP header field\"); for (const unsigned char *part = (const unsigned char *)cursor; part < (const unsigned char *)colon; part += 1) { bool token = (*part >= '0' && *part <= '9') || (*part >= 'A' && *part <= 'Z') || (*part >= 'a' && *part <= 'z') || strchr(\"!#$%&'*+-.^_`|~\", *part) != NULL; if (!token) return flux__net_result(-1, \"invalid HTTP header name\"); } for (const unsigned char *part = (const unsigned char *)(colon + 1); part < (const unsigned char *)next; part += 1) if ((*part < 0x20 && *part != '\\t') || *part == 0x7f) return flux__net_result(-1, \"invalid HTTP header value\"); cursor = next + 2; } response_callback(socket_handle, version, status, reason); cursor = headers; while (!(cursor[0] == '\\r' && cursor[1] == '\\n')) { char *next = strstr(cursor, \"\\r\\n\"); char *colon = memchr(cursor, ':', (size_t)(next - cursor)); *colon = '\\0'; char *value = colon + 1; while (*value == ' ' || *value == '\\t') value += 1; char *value_end = next; while (value_end > value && (value_end[-1] == ' ' || value_end[-1] == '\\t')) value_end -= 1; *value_end = '\\0'; header_callback(socket_handle, cursor, value); cursor = next + 2; } return flux__net_result((int64_t)length, NULL); }\n");
    }
